...
export async function GET(request: Request) {
  ...
  try {
    ...
    const allTasks = tasksRes.rows as unknown as { id: number; name: string }[];
    const completions = completionsRes.rows as unknown as {
      taskName: string;
      date: string;
      userName: string;
      groupName: string;
    ...
    for (const day of interval) {
      ...
      if (groupsOnDuty) {
        if (!requiredTasks.has(dateString)) requiredTasks.set(dateString, []);
    ...
    const fullRecap = Array.from(recapMap.entries())
      .map(([key, value]) => {
    ...
    // --- LOGIKA PAGINASI DI SINI ---
    const totalItems = fullRecap.length;
    const paginatedRecap = fullRecap.slice(offset, offset + limit);
