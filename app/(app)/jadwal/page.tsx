...
import { useSession, isPicketDay, formatTanggalIndo } from "@/components/session-context"
...
export default function JadwalPage() {
  ...
  const allDone = taskViews.length > 0 && taskViews.every((t) => !!t.completedByName)
  ...
  if (!bolehPiket) {
    return (
      <main className="mx-auto grid min-h-[60svh] w-full max-w-3xl place-items-center p-4">
        <p className="text-center text-base">Hari ini bukan jadwal piket untuk grup Anda ({group}). Santai dulu!</p>
      </main>
    )
  }
  ...
  return (
    <main className="mx-auto w-full max-w-3xl space-y-6">
      <header>
        <h1 className="text-balance text-2xl font-semibold tracking-tight">Tugas Piket: {tanggal}</h1>
      </header>
        ...
        <CardContent className="space-y-4">
          <ul className="grid gap-3">
            {taskViews.map((task) => {
              ...
              return (
                <li key={task.id} className="flex items-start justify-between gap-4 rounded-md border p-3">
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <div>
                      <label htmlFor={task.id.toString()} className="cursor-pointer font-medium">
                        {task.name}
                      </label>
          ...
          {!allDone && taskViews.length > 0 && (
            <Alert variant="destructive" className="mt-4">
              ...
              <AlertDescription>Peringatan: Masih ada tugas yang belum diselesaikan!</AlertDescription>
            </Alert>
          )}
        </CardContent>
      </Card>
